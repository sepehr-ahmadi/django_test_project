...
def create_menu_item_database(menu_item_data):
    cur = conn.cursor()
    cur.execute(
        """Insert into menu_item(name,catagory,discount,delevring_time_period,estimated_cooking_time,price) VALUES (%s, %s,%s,%s,%s,%s);""",
        (menu_item_data['name'], menu_item_data['catagory'], menu_item_data['discount'],
         menu_item_data['delevring_time_period'],
...
def update_menu_item_database(menu_item_data):
    cur = conn.cursor()
    cur.execute(
        """UPDATE menu_item SET "name" =%s,"catagory"=%s,"discount"=%s,"delevring_time_period"=%s,"estimated_cooking_time"=%s,"price"=%s  WHERE name =%s""",
        (menu_item_data['name'], menu_item_data['catagory'],menu_item_data['discount'],
         menu_item_data['delevring_time_period'],
         menu_item_data['estimated_cooking_time'],menu_item_data['price'], menu_item_data['name']))
    conn.commit()
    cur.close()
...
def delete_menu_item_database(menu_item_data):
    ...
    cur = conn.cursor()
    cur.execute("""DELETE FROM menu_item WHERE "name"=%s;""", (menu_item_data['name'],));
    conn.commit()
    cur.close()

def read_menu_item_database(menu_item_data):
    cur = conn.cursor()
    if menu_item_data['name'] == "*":
